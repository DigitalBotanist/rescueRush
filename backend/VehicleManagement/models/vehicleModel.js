...
vehicleSchema.statics.registerVehicle = async function (
    vin,
    // todo: fix location
    ...
    if (exists && ["offline", "broken"].includes(exists.status)) {
        ...
        exists &&
        (exists.status === "active" || exists.status === "working")
    ) {
...
vehicleSchema.statics.getNearestVehicleIds = async function (
    longitude,
    latitude
) {
