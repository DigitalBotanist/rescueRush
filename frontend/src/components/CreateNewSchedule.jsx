...
const CreateNewSchedule = () => {
  ...
  useEffect(() => {
    ...
    fetch('/api/resources/drivers', {
      ...
      .then(data => {
        // Ensure data structure matches expected; adjust if necessary
    ...
    fetch('/api/resources/paramedics', {
      ...
      .then(res => {
        if (!res.ok) throw new Error('Failed to fetch paramedics');
        return res.json();
      })
      .then(data => {
        setParamedics(data.users || []);
      })
      .catch(err => console.error('Error fetching paramedics:', err));
  }, [user]); // Added user as dependency

  const handleCreateSchedule = async () => {
    if (!selectedDriver || !selectedParamedic) {
      alert('Please select a driver and paramedic');
      return;
    }
    ...
    try {
      // /api/resources/schedule (previous one)
      const response = await fetch('/api/resources/schedules', {
        method: "POST",
        body: JSON.stringify(scheduleData),
        headers: {
          'Content-Type': 'application/json',
